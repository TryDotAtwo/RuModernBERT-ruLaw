...
\midrule
\base{} & 0{,}99841 & 0{,}99863 & 0{,}99852\\
\model{} & 0{,}99794 & 0{,}99846 & 0{,}99820\\
\bottomrule
\end{tabular}
\end{table}

Тестовые потери исходной и адаптированной моделей практически совпали: $6{,}58\cdot10^{-5}$ и $6{,}57\cdot10^{-5}$. При этом исходная модель получила $F_1=0{,}99852$, а адаптированная --- $F_1=0{,}99820$; разность $-0{,}00032$ не свидетельствует об улучшении NER. Высокие значения нельзя объяснить точными копиями между частями данных: дедупликация выполнена до разбиения, а тексты трёх частей не пересекаются. Следовательно, доказанный эффект доменной адаптации относится к MLM на юридическом тексте, но не переносится автоматически на эту задачу распознавания сущностей.

\section{Заключение}
Продолженное MLM-предобучение русскоязычного ModernBERT на нормативных актах даёт воспроизводимое улучшение на внешней коллекции судебных решений. Парный эксперимент с пятью реализациями маскирования показал уменьшение перплексии на 6{,}45\% с узким доверительным интервалом. На очищенной NER-выборке обе модели достигли $F_1>0{,}998$, однако адаптация не дала улучшения относительно исходного энкодера. Тем самым результат подтверждает доменный выигрыш в языковом моделировании, но показывает необходимость отдельной проверки переноса для каждой прикладной задачи. Модель и код экспериментов опубликованы открыто \cite{modelcard,repository}.
